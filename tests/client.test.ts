...
describe('Convenience helpers', () => {
  ...
  it('isProUser returns true when has active entitlements', async () => {
    const mockData = {
      object: 'list',
    ...
    const result = await client.isProUser('cust_123');

    expect(result).toBe(true);
  });

  it('isProUser returns true when has subscription with gives_access', async () => {
    const entitlementsData = { object: 'list', items: [], next_page: null, url: '' };
    const subscriptionsData = {
      object: 'list',
      items: [{ object: 'subscription', id: 'sub_1', gives_access: true }],
      next_page: null,
      url: '',
    };

    (global.fetch as ReturnType<typeof vi.fn>)
      .mockResolvedValueOnce(createOkResponse(entitlementsData))
      .mockResolvedValueOnce(createOkResponse(subscriptionsData));
  ...
  it('isProUser returns false when no entitlements and no giving subscriptions', async () => {
    const entitlementsData = { object: 'list', items: [], next_page: null, url: '' };
    const subscriptionsData = {
      object: 'list',
      items: [{ object: 'subscription', id: 'sub_1', gives_access: false }],
      next_page: null,
      url: '',
    };
